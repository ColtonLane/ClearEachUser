...
// "keepUsers" are the directories that are skipped over
std::vector<std::string> keepUsers = {
    "colto", "clane", "sday2", "bwhittenbarger", "Administrator", "templocal",
    "Default", "Public", "Default User", "All Users", "astambaugh", "romay",
    "aboggs3", "jwhitt2", "jsturm"};

std::vector<std::string> deleteQueue; // Tracks folders being deleted
std::mutex queueMutex;               // Mutex for synchronizing access to deleteQueue

std::string defaultUserPath = "C:/Users/";
std::string directoryPath;

double startingSpace; // Initial available space in MB
...
void progressBar(const int initialTotal) {
    int deletedCount = 0;
    std::chrono::high_resolution_clock::time_point startTime = std::chrono::high_resolution_clock::now();
    while (deletedCount < initialTotal) {
        {
        ...
        std::cout << "(" << static_cast<int>(timeElapsed) / 60 << ":"
                  << static_cast<int>(timeElapsed) % 60 << ") ";
        std::cout << deletedCount << " out of " << initialTotal << " deleted. ";
        std::cout << "[";
...
// Process AppData for deletion
void removeAppData(const fs::path& folderPath, std::vector<std::thread>& threads) {
    fs::path appDataPath = folderPath / "AppData";
    if (fs::exists(appDataPath)) {
        threads.emplace_back(deleteFolder, appDataPath);
    } else {
        noAppData++;
    }
}
...
int mainLoop() {
    ...
    try {
        fs::path dirPath = directoryPath;
        info = fs::space(dirPath.root_path());
        startingSpace = info.available / bytesToMB;

        std::vector<std::thread> threads; // To manage worker threads
        ...
        for (auto& entry : fs::directory_iterator(directoryPath)) {
            ...
            if (fs::is_directory(p) &&
                std::find(std::begin(keepUsers), std::end(keepUsers), userName) == std::end(keepUsers)) {
                removeAppData(p, threads);
            } else {
                std::cout << "Kept: " << userName << std::endl;
                numUsersKept++;
            }
...
int main() {
    ...
    std::cout << "Deleting the rest of the Users' AppData folders. Keep this window open until that process completes." << std::endl;

    progressBar(deleteQueue.size());

    info = fs::space(fs::path(defaultUserPath).root_path());
    std::cout << "Freed up " << (startingSpace - info.available / bytesToMB) << " MB in "
              << static_cast<int>(timeElapsed) / 60 << ":"
              << static_cast<int>(timeElapsed) % 60 << std::endl;
